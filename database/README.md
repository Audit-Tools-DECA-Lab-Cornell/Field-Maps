...
`hosted/verify.sql` runs thirteen assertions in one transaction: membership isolation, restricted writes, spatial readback, the sample GIS view, and the site-package access rules. It rolls back its synthetic memberships, site, observation, package and temporary administrator role grants. It passed on staging on September 26, 2026. It checks database behavior, not a real Auth session or QGIS Desktop. Run it only as the authorized hosted administrator with stop-on-error enabled.
