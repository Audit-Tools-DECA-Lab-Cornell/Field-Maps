...
FROM pg_namespace WHERE nspname IN ('fieldmaps', 'fieldmaps_meta', 'gis');

SELECT rolname, rolcanlogin, rolsuper, rolcreaterole, rolcreatedb, rolbypassrls
FROM pg_roles WHERE rolname IN ('postgres', 'fieldmaps_api', 'fieldmaps_sample_reader');

SELECT n.nspname AS schema_name, c.relname AS table_name, c.relrowsecurity AS rls_enabled
FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
WHERE n.nspname IN ('public', 'fieldmaps', 'gis') AND c.relkind IN ('r', 'p', 'v')
...
WHERE n.nspname IN ('public', 'extensions', 'fieldmaps', 'gis')
  AND r.rolname IN ('anon', 'authenticated', 'service_role', 'fieldmaps_api');
