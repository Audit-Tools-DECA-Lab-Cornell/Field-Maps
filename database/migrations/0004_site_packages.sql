-- Site packages: what a manager prepares from a QGIS project so an observer can carry a site
-- into a field with no signal.
--
-- A prepared package is a record of a decision, not a working document, so it is immutable in
-- the same way a published form version is: superseding one means preparing the next version
-- beside it. Its checks are kept whether they passed or blocked — a package that was refused is
-- the most useful thing to be able to read six months later.
--
-- The archive lives in this table rather than in object storage. That keeps it in one
-- transaction with its manifest and checks, under the same row-level policies as everything
-- else, and inside the same backup; the cost is the ceiling below. A package is vector geometry
-- with no imagery, so the ceiling is generous for what the format can hold. Moving to object
-- storage later means replacing `archive` with a key and nothing else in this schema.
